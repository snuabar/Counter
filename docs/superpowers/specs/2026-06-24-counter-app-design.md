...
**录制样本**：
1. 进入"样本管理" → 点击"录制样本"
2. 选择传感器类型 → 倒计时 → 开始录制 → 执行动作/发出声音
3. 录制完成后**自动进入裁剪画面**：骨架动画实时预览 + RangeSlider 调整截取范围 + 实时评分
4. 评分及格（≥50分）显示"保存"按钮，不及格显示"重录"按钮
5. 用户可拖动滑块微调截取范围，骨架动画和评分实时响应
6. 命名并保存
