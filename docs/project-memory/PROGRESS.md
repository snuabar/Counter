...
| 30 | 模板录制后实时预览裁剪（骨架动画 + RangeSlider + 实时评分） | 完成 |
...
### Phase 4：检测算法优化（2026-06-29）

| # | 功能 | 状态 |
|---|------|------|
| 35 | **DetectionConfig 配置类** | 完成 |
| 36 | **A：速度/加速度特征**（位置DTW + 速度DTW，权重70%/30%） | 完成 |
| 37 | **B：特征维度自适应加权**（基于模板方差动态分配权重） | 完成 |
| 38 | **D：身高/距离归一化**（bodyHeight / shoulderWidth） | 完成 |
| 39 | **E：自适应平滑窗口**（根据模板帧数自动选择1/3/5窗口） | 完成 |
| 40 | **F：模板在线学习**（检测成功后α混合微调在线模板） | 完成 |
| 41 | **速度匹配度可视化**（velocityScore 0~1 + 颜色渐变进度条） | 完成 |
...
| # | 功能 | 说明 | 预估工作量 |
|---|------|------|----------|
| 1 | **模板录制-计数联调** | TemplateScreen 录制流程、TemplateRecorder、CustomPoseActionDetector 均已实现，录制后新增实时预览裁剪界面（骨架动画+滑块+评分），但尚未在真机验证完整流程：录制模板 → 保存到数据库 → 选择模板计数 → DTW 匹配计数 | 4小时 |
...
| 2026-06-28 | 模板列表交互升级 | 左滑删除 + 点击编辑 + 新增自动录制，动作预览骨架动画 |
| 2026-06-28 | 骨架动画保持比例 | 预览框为长方形时，使用 min(width, height) 统一缩放防止变形 |
| 2026-06-28 | 录制后实时预览裁剪 | 录制完成后自动进入裁剪画面（骨架预览+滑块+评分），评分及格显示"保存"，不及格显示"重录" |
| 2026-06-28 | 动态参数截取 | 根据 peakMotion 自动判断动作类型（快/中/慢），自适应 threshold、minLength、maxLength |
| 2026-06-28 | 谷值跨越策略 | 慢动作允许最多5帧低于 threshold，解决深蹲等慢速动作截取不完整问题 |
| 2026-06-29 | **检测算法可插拔配置** | 新增 DetectionConfig，支持 A/B/D/E/F 五种算法独立开关，便于后期调试 |
| 2026-06-29 | **速度/加速度特征（A）** | 提取速度特征并参与DTW匹配（70%位置 + 30%速度），区分快慢执行 |
| 2026-06-29 | **特征维度自适应加权（B）** | 基于模板各维度方差动态分配权重，变化大的维度权重更高 |
| 2026-06-29 | **身高/距离归一化（D）** | bodyHeight / shoulderWidth 消除拍摄距离影响，默认关闭（谨慎策略） |
| 2026-06-29 | **自适应平滑窗口（E）** | 根据模板帧数自动选择平滑窗口：快动作1帧、中动作3帧、慢动作5帧 |
| 2026-06-29 | **模板在线学习（F）** | 检测成功后使用α=0.1混合微调在线模板，默认关闭 |
| 2026-06-29 | **速度匹配度可视化** | 新增 velocityScore 字段，CountingScreen 显示颜色渐变进度条（绿→黄→红），提示"过快/过慢" |
...
| 5 | `ImageAnalysis.Builder.setTargetResolution()` 已弃用 | 编译警告 | 不影响功能，后续可迁移到 `setResolutionSelector()` |
| 6 | **抬腿录制失败（背手导致手腕遮挡）** | 待处理 | TemplateRecorder 要求12个关键点全部可见，背手时手腕被遮挡导致所有帧被丢弃，需降低关键点可见性要求 |
| 7 | Camera2Preview 未使用参数 | 编译警告 | `isFrontCamera` 和 `modifier` 参数在组件内未使用，功能正常 |
...
**当前会话遗留（会话8）：**
1. 抬腿录制失败问题（背手导致手腕关键点被遮挡，需降低录制关键点可见性要求或允许部分点缺失）
2. 音频检测引擎真机调优
3. 计时反馈增强（目标提醒、语音播报、庆祝动画）
